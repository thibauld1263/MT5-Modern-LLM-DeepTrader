...
input double   InpCloseConfidence= 0.40;       // Close trade if confidence drops below this
input double   InpAbortThreshold= 0.60;        // Close trade if Abort probability hits this
...
void OnTick()
{
   ...
   static int bars_time_filtered = 0;
   static int bars_atr_fail = 0;
   static int bars_input_fail = 0;
   static int bars_onnx_fail = 0;
   static int bars_inference_ok = 0;
   bars_total++;
   ...
   if(dt.hour < InpStartHour || dt.hour >= InpEndHour)
   {
      bars_time_filtered++;
      if(bars_total % 500 == 0)
         Print(StringFormat("[DIAG] Bars: %d | TimeFilter: %d | ATRFail: %d | InputFail: %d | OnnxFail: %d | OK: %d",
               bars_total, bars_time_filtered, bars_atr_fail, bars_input_fail, bars_onnx_fail, bars_inference_ok));
      return;
   }
   ...
   // Get ATR manually (more reliable than iATR in Strategy Tester)
   MqlRates atr_rates[];
   int atr_copied = CopyRates(_Symbol, PERIOD_M30, 1, 15, atr_rates); // 14 bars + 1 for prev close
   if(atr_copied < 15)
   {
      bars_atr_fail++;
      return;
   ...
   for(int i = 1; i < 15; i++)
   {
      double tr = MathMax(atr_rates[i].high - atr_rates[i].low,
                  MathMax(MathAbs(atr_rates[i].high - atr_rates[i-1].close),
                          MathAbs(atr_rates[i].low  - atr_rates[i-1].close)));
      tr_sum += tr;
   }
   double current_atr = tr_sum / 14.0;
   ...
   // DEBUG: Print model output on every bar
   Print(StringFormat("  [DEBUG] P_long=%.4f  P_short=%.4f  P_abort=%.4f", p_long, p_short, p_abort));

   // Dynamically manage existing trades before look for new entries
   ManageOpenTrades(p_long, p_short, p_abort);

   // Check signal for new entries
...
//+------------------------------------------------------------------+
//| Manage open trades: close them if model signals danger             |
//+------------------------------------------------------------------+
void ManageOpenTrades(double p_long, double p_short, double p_abort)
{
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      if(PositionGetSymbol(i) == _Symbol && PositionGetInteger(POSITION_MAGIC) == InpMagicNumber)
      {
         ...
         if(p_abort >= InpAbortThreshold)
         {
            should_close = true;
            close_reason = "Abort Prob High";
         }
         else if(type == POSITION_TYPE_BUY && p_long < InpCloseConfidence)
         {
            should_close = true;
            close_reason = "Long Prob Dropped";
         }
         else if(type == POSITION_TYPE_SELL && p_short < InpCloseConfidence)
         {
            should_close = true;
            close_reason = "Short Prob Dropped";
         }
         ...
         if(should_close)
         {
            ...
            request.comment = "Closed: " + close_reason;
            request.type_filling = ORDER_FILLING_IOC;

            if(OrderSend(request, result))
            {
               Print("  ✗ Closed ", (type == POSITION_TYPE_BUY ? "BUY" : "SELL"), " #", ticket, " Reason: ", close_reason);
            }
         }
      }
   }
